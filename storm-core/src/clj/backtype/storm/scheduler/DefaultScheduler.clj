...
(ns backtype.storm.scheduler.DefaultScheduler
  ...
  (:require [backtype.storm.scheduler.EasyScheduler :as EasyScheduler])
  (:import [backtype.storm.scheduler IScheduler Topologies
            Cluster TopologyDetails WorkerSlot SchedulerAssignment
            EvenScheduler EasyScheduler ExecutorDetails])
  (:gen-class
...
(defn default-schedule [^Topologies topologies ^Cluster cluster]
  (let [needs-scheduling-topologies (.needsSchedulingTopologies cluster topologies)]
    (doseq [^TopologyDetails topology needs-scheduling-topologies
      ...
      (EasyScheduler/schedule-topologies-easily (Topologies. {topology-id topology}) cluster)
      )))
