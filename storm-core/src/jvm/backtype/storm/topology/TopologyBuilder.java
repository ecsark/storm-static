...
public class TopologyBuilder {
    ...
    private Map<String, IRichBolt> _statebolts = new HashMap<String, IRichBolt>();
...
//    private Map<String, Map<GlobalStreamId, Grouping>> _inputs = new HashMap<String, Map<GlobalStreamId, Grouping>>();
    ...
    public StormTopology createTopology() {
        ...
        Map<String, Bolt> stateBoltSpecs = new HashMap<String, Bolt>();
        ...
        for(String boltId: _statebolts.keySet()) {
            IRichBolt bolt = _statebolts.get(boltId);
            ComponentCommon common = getComponentCommon(boltId, bolt);
            stateBoltSpecs.put(boltId, new Bolt(ComponentObject.serialized_java(Utils.serialize(bolt)), common));
        }
    ...
    public BoltDeclarer setStateBolt(String id, IRichBolt bolt) {
        return setStateBolt(id, bolt, null);
    }

    public BoltDeclarer setStateBolt(String id, IRichBolt bolt, Number parallelism_hint) {
        validateUnusedId(id);
        initCommon(id, bolt, parallelism_hint);
        _statebolts.put(id, bolt);
        return new BoltGetter(id);
    }

    public BoltDeclarer setStateBolt(String id, IBasicBolt bolt) {
        return setStateBolt(id, bolt, null);
    }

    public BoltDeclarer setStateBolt(String id, IBasicBolt bolt, Number parallelism_hint) {
        return setStateBolt(id, new BasicBoltExecutor(bolt), parallelism_hint);
    }
    ...
    private void validateUnusedId(String id) {
        ...
        if(_statebolts.containsKey(id)) {
            throw new IllegalArgumentException("Stateless bolt has already been declared for id " + id);
        }
